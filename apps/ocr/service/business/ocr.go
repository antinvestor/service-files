...
type OCRBusiness interface {
	Recognise(ctx context.Context, request *ocrv1.RecognizeRequest) (*ocrv1.RecognizeResponse, error)
...
type Recognizer interface {
	Recognise(ctx context.Context, image *os.File) (string, error)
}
...
func (ob *ocrBusiness) Recognise(ctx context.Context, request *ocrv1.RecognizeRequest) (*ocrv1.RecognizeResponse, error) {
	...
	for _, fileID := range request.GetFileId() {
		...
		if request.GetAsync() {
			...
			newOcrLog, err = ob.recognise(ctx, newOcrLog)
			if err != nil {
				return nil, err
			}
		}
...
func (ob *ocrBusiness) recognise(ctx context.Context, ocrLog *models.OcrLog) (*models.OcrLog, error) {
